...
if (!UserUtil::hasPermission($_SESSION['username'], 'admin.product.code.view')) {
    echo 'no_permission';
    exit();
}
?>
...
<div class="admin-header">
    <div class="admin-header-title mr-auto"> Product Code Manager </div>
...
<div class="admin-body"> 
    <div class="admin-table">
        <div class="admin-table-header">
            <?php include(LOCAL_PATH_ROOT.'/game/admin/product_code/table_header.php');?>
        </div>

        <div class="admin-table-scroll">
            <?php include(LOCAL_PATH_ROOT.'/game/admin/product_code/table_scroll.php');?>
        </div>
    </div>
</div>

<div class="admin-footer">
    <select id="product-code-in-page">
...
<script>
    var postData = {
        ...
        "searchFromAddedDate": '<?=$searchFromAddedDate?>',
        "searchToAddedDate": '<?=$searchToAddedDate?>',
        "currentPage": <?=$currentPage?>,
        "maxInPage": <?=$maxInPage?>
    };

    $("#product-code-in-page").change(function(e) {
    ...
    function reloadProductCodeAdmin() {
        $.ajax({
            url: "product_code/index.php", 
            method: "POST",
            data: postData,
    ...
    function getProductCodeFromCheckboxs() {
        let productCodeArray = [];
        $(".admin-checkbox:not(.all)").each(function() {
            if ($(this)[0].checked) {
                productCodeArray.push(this.dataset.name);
            }
        });
    ...
    $("#admin-header-search").click(function(e) {
        $.ajax({
            url: "product_code/search_product_code_form.php",
            method: "POST",
            data: postData,
            success: function(result){
    ...
    $("#admin-header-add").click(function(e) {
        $.ajax({
            url: "product_code/add_product_code_form.php",
            success: function(result){
                if (result == 'no_permission') {
                    window.alert("You do not have permission to use this!");
                }else {
    ...
    $("#admin-header-edit").click(function(e) {
        let codeIdArray = getProductCodeFromCheckboxs();

        if (codeIdArray.length != 1) {
            alert("You must choose one row!");
            return;
        }

        let codeId = codeIdArray[0];

        $.ajax({
            url: "product_code/edit_product_code_form.php",
            method: "POST",
            data: {
                "id": codeId
            },
    ...
    $("#admin-header-remove").click(function (e) {
        ...
        showLoading();
        $.ajax({
            url: "product_code/remove_product_code.php", 
            method: "POST",
            data: {
                "code_id_array": codeIdArray
            },
            success: function(result){
                if (result == 'no_permission') {
                    window.alert("You do not have permission to use this!");
                }else if (result == 0) {
                    callNotice("Remove Product Code Successfully!");
                    updateSideBar();
