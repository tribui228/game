...
if (!UserUtil::hasPermission($_SESSION['username'], 'admin.user.view')) {
    echo 'no_permission';
    exit();
}
?>
...
<div class="admin-header">
    <div class="admin-header-title mr-auto"> User Manager </div>
...
<div class="admin-body"> 
    <div class="admin-table">
        <div class="admin-table-header">
            <?php include(LOCAL_PATH_ROOT.'/game/admin/user/table_header.php');?>
        </div>

        <div class="admin-table-scroll">
            <?php include(LOCAL_PATH_ROOT.'/game/admin/user/table_scroll.php');?>
        </div>
    </div>
</div>

<div class="admin-footer">
    <select id="user-in-page">
...
<script>
    var postData = {
        ...
        "searchFromBirthDate": '<?=$searchFromBirthDate?>',
        "searchToBirthDate": '<?=$searchToBirthDate?>',
    ...
    $("#user-in-page").change(function(e) {
    ...
    function getUserNamesFromCheckboxs() {
        let userNames = [];
        $(".admin-checkbox:not(.all)").each(function() {
            if ($(this)[0].checked) {
                userNames.push(this.dataset.name);
            }
        });
        return userNames;
    }

    var userColumnSortBy = <?php echo is_null($sortBy) ? 'null' : "'$sortBy'";?>;
    var userColumnSortDir = <?php echo is_null($sortDir) ? 'null' : "'$sortDir'";?>;
    $('.sort-column').click(function(e) {
        let column = e.currentTarget.dataset.column;

        if (column != userColumnSortBy) {
            userColumnSortDir = null;
            $arrow = $('#admin-arrow-' + userColumnSortBy);
            $arrow.remove();
            $(this).append('<div id="admin-arrow-' + column + '" class="admin-arrow arrow down"> </div>');
        }else {
            $('#admin-arrow-' + userColumnSortBy).toggleClass('down', userColumnSortDir != 'ASC');
            $('#admin-arrow-' + userColumnSortBy).toggleClass('up', userColumnSortDir == 'ASC');
        }
        userColumnSortBy = column;
        userColumnSortDir = userColumnSortDir == null ? 'ASC' : (userColumnSortDir == 'ASC' ? 'DESC' : 'ASC');

        postData.sortBy = userColumnSortBy;
        postData.sortDir = userColumnSortDir;
		$.ajax({
            url: "user/table_scroll.php",
            method: "POST",
            data: postData,
            success: function(result){
    ...
    $("#admin-header-edit").click(function(e) {
        let userNames = getUserNamesFromCheckboxs();
    ...
    $("#admin-header-remove").click(function (e) {
        ...
        $.ajax({
            ...
            method: "POST",
            data: {"username_array": userNames},
            success: function(result){
                if (result == 'no_permission') {
                    window.alert("You do not have permission to use this!");
                }else if (result == 0) {
                    callNotice("Remove User Successfully!");
                    updateSideBar();
                    reloadUserAdmin();
                }else if (result == 1) {
                    callNotice("Have user cannot remove!");
                } else if (result == 2) {
