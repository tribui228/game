...
if (!UserUtil::hasPermission($_SESSION['username'], 'admin.user.edit')) {
    echo 'no_permission';
    exit();
}
...
<div id="modal-header"> Edit User</div>
        ...
        <div class="container p-0">
            <div class="row">
                <div class="col-12 col-md-6 form-group">
                    ...
                    <input type="text" class="form-control" id="user-username" value="<?=$username?>" readonly>
                ...
                <div class="col-12 col-md-6 form-group">
                    ...
                    <select id="user-group" class="form-select">
                        ...
                        ?>
                            <option value="<?=$name?>" <?php echo $name == $groupName ? 'selected' : ''?>> <?=$display?> </option>
                        <?php endforeach; ?>
                    </select>
...
<div class="col-12 form-group mt-3 pr-0 pl-0">
        <button type="button" class="form-control" id="user-edit-permission" form="edit-user-form">
            Edit User Permission
        </button>   
    </div>
    <div class="col-12 form-group mb-0 pr-0 pl-0">
        <button type="submit" class="form-control admin-submit" id="user-confirm" form="edit-user-form">
            Save User
        </button>   
    </div>
</div>
...
<script>
    $("#user-edit-permission").click(function() {
        $.ajax({
        ...
        if (updateUserUsername() && updateUserPassword() && updateUserConfirmPassword() && updateUserFirstname() && updateUserLastname() && updateUserEmail() && updateUserPhone() && updateUserBirth() && updateUserAddress()) {
            ...
            $.ajax({
                ...
                success: function(response){
                    if (response == 0) {
                        callNotice("Edit User Successfully!");
