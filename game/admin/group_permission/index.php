...
include_once(LOCAL_PATH_ROOT.'/game/admin/group_permission/params.php');

if(session_status() == PHP_SESSION_NONE){    
    session_start();
}

if (!UserUtil::hasPermission($_SESSION['username'], 'admin.group.permission.view')) {
    echo 'no_permission';
    exit();
}
...
<div class="admin-header">
    <div class="admin-header-title mr-auto"> Permission Manager By Group <?=$_POST['group']?> </div>
...
<div class="admin-body"> 
    <div class="admin-table">
        <div class="admin-table-header">
            <?php include(LOCAL_PATH_ROOT.'/game/admin/group_permission/table_header.php');?>
        </div>

        <div class="admin-table-scroll">
            <?php include(LOCAL_PATH_ROOT.'/game/admin/group_permission/table_scroll.php');?>
        </div>
    </div>
</div>

<script>
    function reloadGroupPermissionAdmin() {
        $.ajax({
    ...
    function getIdFromCheckboxs() {
        let idArray = [];
        $(".admin-checkbox:not(.all)").each(function() {
            if ($(this)[0].checked) {
                idArray.push(this.dataset.name);
            }
        });
        return idArray;
    }

    var groupPermissionName = "<?=$_POST['group']?>";
    var groupColumnSortBy = <?php echo is_null($sortBy) ? 'null' : "'$sortBy'";?>;
    var groupColumnSortDir = <?php echo is_null($sortDir) ? 'null' : "'$sortDir'";?>;
    $('.sort-column').click(function(e) {
        let column = e.currentTarget.dataset.column;

        if (column != groupColumnSortBy) {
            groupColumnSortDir = null;
            $arrow = $('#admin-arrow-' + groupColumnSortBy);
            $arrow.remove();
            $(this).append('<div id="admin-arrow-' + column + '" class="admin-arrow arrow down"> </div>');
        }else {
            $('#admin-arrow-' + groupColumnSortBy).toggleClass('down', groupColumnSortDir != 'ASC');
            $('#admin-arrow-' + groupColumnSortBy).toggleClass('up', groupColumnSortDir == 'ASC');
        }
        groupColumnSortBy = column;
        groupColumnSortDir = groupColumnSortDir == null ? 'ASC' : (groupColumnSortDir == 'ASC' ? 'DESC' : 'ASC');

		$.ajax({
            url: "group_permission/table_scroll.php",
            method: "POST",
            data: {
                "group": groupPermissionName,
                "sortBy": groupColumnSortBy,
                "sortDir": groupColumnSortDir,
            },
            success: function(result){
    ...
    $("#admin-header-add").click(function(e) {
        $.ajax({
            url: "group_permission/add_group_permission_form.php",
            method: "POST",
            data: {
                "group": groupPermissionName
            },
    ...
    $("#admin-header-edit").click(function(e) {
        let idArray = getIdFromCheckboxs();
        ...
        $.ajax({
            url: "group_permission/edit_group_permission_form.php",
            method: "POST",
            data: {"id": name},
            success: function(result){
                if (result == 'no_permission') {
    ...
    $("#admin-header-remove").click(function (e) {
        ...
        showLoading();
        $.ajax({
            url: "group_permission/remove_group_permission.php", 
            method: "POST",
            ...
            success: function(result){
                if (result == 'no_permission') {
                    ...
                    callNotice("Remove Group Permission Successfully!");
                    reloadGroupPermissionAdmin();
                }
                closeLoading();
            }
        });
