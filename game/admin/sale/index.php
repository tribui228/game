...
if (!UserUtil::hasPermission($_SESSION['username'], 'admin.sale.view')) {
    echo 'no_permission';
    exit();
}
?>
...
<div class="admin-header">
    <div class="admin-header-title mr-auto"> Sale Manager </div>
...
<div class="admin-body"> 
    <div class="admin-table">
        <div class="admin-table-header">
            <?php include(LOCAL_PATH_ROOT.'/game/admin/sale/table_header.php');?>
        </div>

        <div class="admin-table-scroll">
            <?php include(LOCAL_PATH_ROOT.'/game/admin/sale/table_scroll.php');?>
        </div>
    </div>
</div>
...
<script>
    var postData = {
        ...
        "searchMinDiscount": '<?=$searchMinDiscount?>',
        "searchMaxDiscount": '<?=$searchMaxDiscount?>',
        "searchFromBeginDate": '<?=$searchFromBeginDate?>',
        "searchToBeginDate": '<?=$searchToBeginDate?>',
        "searchFromEndDate": '<?=$searchFromEndDate?>',
        "searchToEndDate": '<?=$searchToEndDate?>',
        "currentPage": <?=$currentPage?>,
        "maxInPage": <?=$maxInPage?>
    };

    $("#sale-in-page").change(function(e) {
    ...
    function getSaleNamesFromCheckboxs() {
        let saleNames = [];
        $(".admin-checkbox:not(.all)").each(function() {
            if ($(this)[0].checked) {
                saleNames.push(this.dataset.name);
            }
        });
        return saleNames;
    }

    var saleColumnSortBy = <?php echo is_null($sortBy) ? 'null' : "'$sortBy'";?>;
    var saleColumnSortDir = <?php echo is_null($sortDir) ? 'null' : "'$sortDir'";?>;
    $('.sort-column').click(function(e) {
        let column = e.currentTarget.dataset.column;

        if (column != saleColumnSortBy) {
            saleColumnSortDir = null;
            $arrow = $('#admin-arrow-' + saleColumnSortBy);
            $arrow.remove();
            $(this).append('<div id="admin-arrow-' + column + '" class="admin-arrow arrow down"> </div>');
        }else {
            $('#admin-arrow-' + saleColumnSortBy).toggleClass('down', saleColumnSortDir != 'ASC');
            $('#admin-arrow-' + saleColumnSortBy).toggleClass('up', saleColumnSortDir == 'ASC');
        }
        saleColumnSortBy = column;
        saleColumnSortDir = saleColumnSortDir == null ? 'ASC' : (saleColumnSortDir == 'ASC' ? 'DESC' : 'ASC');

        postData.sortBy = saleColumnSortBy;
        postData.sortDir = saleColumnSortDir;
		$.ajax({
            url: "sale/table_scroll.php",
            method: "POST",
            data: postData,
            success: function(result){
    ...
    $("#admin-header-edit").click(function(e) {
        let saleNames = getSaleNamesFromCheckboxs();
        ...
        let name = saleNames[0];

        $.ajax({
            url: "sale/edit_sale_form.php",
            method: "POST",
            data: {"name": name},
            success: function(result){
                if (result == 'no_permission') {
    ...
    $("#admin-header-remove").click(function (e) {
        ...
        showLoading();
        $.ajax({
            url: "sale/remove_sale.php", 
            method: "POST",
            data: {"sale_names": saleNames},
            success: function(result){
                if (result == 'no_permission') {
                    window.alert("You do not have permission to use this!");
                }else if (result == 0) {
                    callNotice("Remove Sale Successfully!");
                    updateSideBar();
                    reloadSaleAdmin();
                }else if (result == 1) {
                    ...
                    callNotice("Delete all product use this sale first!");
                }else if (result == 3) {
