...
if (!UserUtil::hasPermission($_SESSION['username'], 'admin.group.view')) {
    echo 'no_permission';
    exit();
}
?>
...
<div class="admin-header">
    <div class="admin-header-title mr-auto"> Group Manager </div>
...
<div class="admin-body"> 
    <div class="admin-table">
        <div class="admin-table-header">
            <?php include(LOCAL_PATH_ROOT.'/game/admin/group/table_header.php');?>
        </div>

        <div class="admin-table-scroll">
            <?php include(LOCAL_PATH_ROOT.'/game/admin/group/table_scroll.php');?>
        </div>
    </div>
</div>

<div class="admin-footer">
    <select id="group-in-page">
...
<script>
    ...
    $("#group-in-page").change(function(e) {
    ...
    function getGroupNamesFromCheckboxs() {
        let groupNames = [];
        $(".admin-checkbox:not(.all)").each(function() {
            if ($(this)[0].checked) {
                groupNames.push(this.dataset.name);
            }
        });
        return groupNames;
    }
    ...
    $('.sort-column').click(function(e) {
        ...
        postData.sortBy = groupColumnSortBy;
        postData.sortDir = groupColumnSortDir;
		$.ajax({
            url: "group/table_scroll.php",
            method: "POST",
            data: postData,
            success: function(result){
    ...
    $("#admin-header-search").click(function(e) {
        $.ajax({
            url: "group/search_group_form.php",
            method: "POST",
            data: postData,
            success: function(result){
    ...
    $("#admin-header-edit").click(function(e) {
        let groupNames = getGroupNamesFromCheckboxs();
        ...
        let name = groupNames[0];

        $.ajax({
            url: "group/edit_group_form.php",
            method: "POST",
            data: {"group": name},
            success: function(result){
                if (result == 'no_permission') {
    ...
    $("#admin-header-remove").click(function (e) {
        ...
        showLoading();
        $.ajax({
            url: "group/remove_group.php", 
            method: "POST",
            data: {"group_array": groupNames},
            success: function(result){
                if (result == 'no_permission') {
                    window.alert("You do not have permission to use this!");
                }else if (result == 0) {
                    callNotice("Remove Group Successfully!");
                    updateSideBar();
                    reloadGroupAdmin();
                }else if (result == 1) {
                    ...
                    callNotice("Cannot delete this group!");
                } else if (result == 3) {
