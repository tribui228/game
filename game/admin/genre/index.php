...
if (!UserUtil::hasPermission($_SESSION['username'], 'admin.genre.view')) {
    echo 'no_permission';
    exit();
}
?>
...
<div class="admin-header">
    <div class="admin-header-title mr-auto"> Genre Manager </div>
...
<div class="admin-body"> 
    <div class="admin-table">
        <div class="admin-table-header">
            <?php include(LOCAL_PATH_ROOT.'/game/admin/genre/table_header.php');?>
        </div>

        <div class="admin-table-scroll">
            <?php include(LOCAL_PATH_ROOT.'/game/admin/genre/table_scroll.php');?>
        </div>
    </div>
</div>
...
<script>
    ...
    function getGenreNamesFromCheckboxs() {
        let genreNames = [];
        $(".admin-checkbox:not(.all)").each(function() {
            if ($(this)[0].checked) {
                genreNames.push(this.dataset.name);
            }
        });
        return genreNames;
    }
    ...
    $("#admin-header-search").click(function(e) {
        $.ajax({
            url: "genre/search_genre_form.php",
            method: "POST",
            data: postData,
            success: function(result){
    ...
    $("#admin-header-edit").click(function(e) {
        let genreNames = getGenreNamesFromCheckboxs();
        ...
        let name = genreNames[0];

        $.ajax({
            url: "genre/edit_genre_form.php",
            method: "POST",
            data: {"name": name},
            success: function(result){
                if (result == 'no_permission') {
        ...
        $.ajax({
            ...
            success: function(result){
                if (result == 'no_permission') {
                    ...
                    callNotice("Remove Genre Successfully!");
                    updateSideBar();
                    reloadGenreAdmin();
                }else if (result == 1) {
                    ...
                    callNotice("Delete all product of this genre first!");
                }else if (result == 3) {
